 $(document).ready(function () {
    const add_copy_btn = function () {
      $("pre").each(function (index) {
        ...
        var $div = $("<div id='" + id + "' class='kapow-copy' style='position: relative;'>");
        var $button = $(
          "<div class='kapow-copy-btn' data-id='" +
            id +
            "' style='color: #ffffff; cursor: pointer; position: absolute; top: 8px; right: 8px; background-color: #173077; padding: 0.5rem 0.8rem; min-width: 100px; max-width: 120px; text-align: center; font-size: 0.9rem; line-height: 1.2rem; border-radius: 0.375rem;'>"
        ).appendTo($div);
        var svg_clip =
        ...
        var button_html =
          "<span class='copy-icon' style='display: flex;'>" +
          svg_clip +
          "</span><span class='copy-text' style='padding-left: 5px;'>Copy Code</span>";
        $button.html(button_html);
        $(this).before($div);

        $div.find(".kapow-copy-btn").click(function () {
          var id = $(this).data("id");
